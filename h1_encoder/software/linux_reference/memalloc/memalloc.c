    ...
    for(i = 0; i < max_chunks; i++)
    {
        if(hlina_chunks[i].file_id == *((int *) (filp->private_data)))
        {
            if (hlina_chunks[i].used) {
                printk("memalloc: Unreleased at close, chunk %d at 0x%lx size %d\n",
...
#error HLINA_END_ADDRESS and/or HLINA_START_ADDRESS not valid!
    ...
    printk("memalloc: linear memory base = 0x%lx \n", HLINA_START_ADDRESS);
    printk("memalloc: linear memory end  = 0x%lx \n", HLINA_END_ADDRESS);
...
static int AllocMemory(unsigned *busaddr, unsigned int size, struct file *filp)
{
    ...
    *busaddr = 0;
    ...
    if (*busaddr == 0)
    {
        size_t nextba = hlina_chunks[max_chunks-1].bus_address + hlina_chunks[max_chunks-1].size;
        size_t memavail = HLINA_END_ADDRESS - nextba;
        ...
        if ((memavail >= chunksize) && (max_chunks < MAX_CHUNKS))
        {
            *busaddr = hlina_chunks[max_chunks].bus_address = nextba;
            ...
            PDEBUG("memalloc: Created new chunk %d at 0x%lx of size = %d\n",
...
void ResetMems(void)
{
    ...
    {
        printk("memalloc: WARNING! Linear memory top limit 0x%lx exceeded!\n",
                HLINA_END_ADDRESS);
    }
